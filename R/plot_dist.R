...
plot_dist <- function(dist, labels=c(), scale = 1, color="skyblue", label_col = "black", plot_dist_name=T) {
  ...
  if(plot_dist_name) {
    text(grconvertX(dist$name_pos[1], from="npc"), grconvertY(dist$name_pos[2], from="npc"), dist$name, cex=1.5 * scale, col = label_col)
  }
  ...
  if(is.character(names(labels))) {
    for(label_name in names(labels)) {
      ...
      text(grconvertX(xpos, from="npc"), grconvertY(ypos, from="npc"), label, cex=2 * scale, col = label_col)
    }
  } else {
