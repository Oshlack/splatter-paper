...
sc3s <- lapply(1:length(sims), function(i) {
    ...
    sim <- sims[[i]]
    sc3 <- sc3_prepare(sim, ks = 3, n_cores = 1, gene_filter = FALSE)
...
```{r metric-median}
median(metrics$DE_True)
median(metrics$DE_Predicted)
median(metrics$Mk_True)
median(metrics$Mk_Predicted)
```
