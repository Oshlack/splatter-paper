...
This is a lot of datasets to consider so we are going to select a subset that
covers different species, cell types, platforms, processing etc.

```{r datasets-selected}
selected <- c(2, 5, 7, 8, 11)
datasets[selected, ]
```
...
for (idx in selected) {
    ...
    save_plot(paste0("../output/overall_", name, ".png"), plt,
              ncol = 4, nrow = 7)
    message("All done!")
}
```
...
We also produce a supplementary table of the MADs.
...
summary %>%
    ...
    write_lines("../output/mads_table.tex")
```
